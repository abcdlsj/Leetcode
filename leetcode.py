...
class Leetcode:
    ...
    def get_source(self, url):  # /submissions/detail/14313499/
        ...
        try:
            src = re.search('submissionCode: \'(.*)\',',
                            requests.get(req_url,
                                         headers=self.headers).text).group(1)
    ...
    def output_source(self, lang='cpp', lang_suffix='cpp', max_threads=8):
        ...
        for idx, question in enumerate(solved_list):
            print("processing: {}. {} ({}/{})".format(
                question["question_id"], question["question_title"], idx + 1,
                len(solved_list)))
            ...
            def process_submit_list(question_):
                ...
                for submit in submit_list:
                    if submit["lang"] == lang:
                        ...
                        dir_name = "n{:04d}.{}".format(question_["question_id"],
            ...
            map(
                lambda u: "- [{}]({})".format(
                    ...
                    requote_uri((Leetcode.REPO_URL + '/tree/master/{}'.format(
                        u.replace('*', ''))).strip())), question_list))
        with open("README.md", "w", encoding='utf-8') as f:
            f.write(
                REPO_README_TEMPLATE.format(solv_question_num=sum(
                    question_level.values()),
...
if __name__ == '__main__':
    ...
    subprocess.run([
        "git", "commit", "-m", "commit by leetcode_.py @abcdlsj at {}".format(
